...
if (!isset($_SESSION['studentID'])) {
    header('Location: student-login.php'); // Redirect to the student login page if not logged in
    exit();
}
...
if (isset($_POST['submit-vehicle'])) {
    ...
    // Insert data into the student_pending table
    $query = mysqli_query($con, "INSERT INTO student_pending(StudentID, studentName, VehicleCategory, VehicleCompanyname, RegistrationNumber, OwnerName, OwnerContactNumber, CarGrant, Status)
                                VALUES('$studentID', '$studentName', '$catename','$vehcomp','$vehreno','$ownername','$ownercontno', '$carGrantFileName', 'Pending')");

    if ($query) {
        echo "<script>alert('Vehicle Entry Detail has been added for student');</script>";
        echo "<script>window.location.href ='s_register.php'</script>";
    } else {
                ...
                <div class="col-md-12">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" value="<?php echo $studentID; ?>" readonly>
                        </div>

                        <!-- Student Name input (read-only) -->
                        <div class="form-group">
                            <label>Student Name</label>
                            <input type="text" class="form-control" value="<?php echo $studentName; ?>" readonly>
                        </div>
